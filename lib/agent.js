...
class Agent extends OriginalAgent {
  ...
  addRequest(req, options, ...args) {
    if (this.options.testOnBorrow) {
      // getName() needs an object, but options might be a string (legacy API).
      // Normalize only for our lookup — super handles its own normalization.
      const opts = typeof options === 'string'
        ? { host: options, port: args[0], localAddress: args[1] }
        : options;
      this._purgeStaleFreeSockets(this.getName(opts));
    }
